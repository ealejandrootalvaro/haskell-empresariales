...
getReservationsByDate conn rango = do
    result <- (D.query conn "select id_reservation, user_restaurant,table_restaurant, date_init AT TIME ZONE 'CDT', date_end AT TIME ZONE 'CDT', amount_people, state from reservation where date_init >= ? and date_init <= ? and state = 4 " ((Reservations.fromInitialDate rango),(Reservations.toFinalDate rango)) :: IO [Reservations.Reservation])
    return result
